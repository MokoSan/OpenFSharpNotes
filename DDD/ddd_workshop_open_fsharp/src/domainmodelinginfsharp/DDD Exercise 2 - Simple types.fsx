...
module SimpleTypes =
    ...
    type UsdMoney   = UsdMoney of float 
    type GpbMoney   = GpbMoney of float 
    ...
    module WidgetCode =
        let create (s:string) = 
            if String.IsNullOrEmpty(s) then None
            else if s.StartsWith("W") then Some ( WidgetCode s )
            else None
    ...
    module GizmoCode = 
        let create ( s : string ) = 
            if String.IsNullOrEmpty( s ) then None
            else if s.StartsWith("G") then Some( GizmoCode s )
            else None
        let get ( GizmoCode c ) = c
